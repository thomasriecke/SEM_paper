...
cat("
    ...
    for (t in 1:(n.years-1)){
      kappa[t] ~ dnorm(0, tau.kappa)     # survival random effect
    }
    for (t in 1:n.years){
      omega[t] ~ dnorm(0, tau.omega)     # fecundity random effect
    }
    ...
    for (i in 1:nI){
      ...
      for (t in f[i]:(n.years-1)){
        log(h[i,t]) = alpha.phi + beta.phi * (a[i] - f[i] + t - 1) + 
                      gamma.phi * eta[i] + kappa[t]
        phi[i,t] = exp(-h[i,t])
...
save.image("O:/PIFL_WTDI_senescence_heterogeneity/results/250k_pifl_latent.RData")
...
###
par(mfrow = c(1,2), oma = rep(0,4), family = 'serif', mar = c(5.1,5.1,2.1,2.1))
plot((exp(-exp(m.fl$q50$alpha.phi + m.fl$q50$beta.phi*0:8))),
